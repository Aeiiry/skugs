...
def setRandomWindowIcon(window: QMainWindow) -> str:
    if iconList := os.listdir("data/program_icons"):
        icon: str = iconList[math.floor(random.random() * len(iconList))]

        window.setWindowIcon(QIcon(f"data/program_icons/{icon}"))
    else:
...
def main() -> None:
    ...
    skugfont: QFont = importFont("data/fonts/setznick-nf/SelznickRemixNF.ttf", 20)

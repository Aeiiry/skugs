...
def get_frame_data_for_move(
    ...
    # Attempt to turn move name into str, if it is not already
    # if it cannot be turned into a string, return an empty dataframe
    try:
        move_name = str(move_name)
    except Exception:
        logger.debug(f"Could not turn move name [{move_name}] into a string")
        return pandas.DataFrame()
...
def find_generic_move_data(
    ...
    if match := re.search(generic_move_name_regex, move_name, flags=re.IGNORECASE):
        # get the generic move name
        generic_move_name: str = match[1] + match[3]

        # search for the generic move name in the frame data dataframe
        logger.debug(f"Data for move [{move_name}] found as [{generic_move_name}]")
        return find_move_from_name_and_character(
...
def find_alias_move_data(
    ...
    Returns:
        pandas.DataFrame: Dataframe with move data, or empty dataframe if move not found
    """
    if move_name_alias := get_alias_move(move_name, move_name_alias_df):
...
def get_alias_move(move_name: str, move_name_alias_df: pandas.DataFrame) -> str:
    ...
    return "" if alias_df.empty else alias_df["Key"].iloc[0]
...
def get_frame_data_for_combo(
    ...
    combo_moves: pandas.Series = combo_df[const.MOVE_NAME]
...
def clean_and_extract_damage(
    ...
    for _, string in enumerate(dmg_list):
        ...
        if numhits_result := re.search(num_hits_regex, string):
            numhits: int = int(numhits_result[2])
            hitdmg: int = int(numhits_result[1])
...
def extract_values_from_brackets(lst: list[str], string: str) -> tuple[list[str], str]:
    ...
    if re.search(bracket_regex, string):
        if r := re.search(bracket_regex, string):
            lst.append(r[0])
...
def extract_values_from_parentheses(
    ...
    if re.search(parentheses_regex, string):
        if r := re.search(parentheses_regex, string):
            lst.append(r[0])

...
def get_combo_damage(combo_frame_data_df: DataFrame) -> DataFrame:
    ...
    df_newhits: DataFrame = parse_hits(combo_frame_data_df)
...
csv_list: list[str] = get_csv_list(fm.CSV_PATH)
...
for csv in csv_list:
    ...
    combo_framedata_df = pd.concat(
        [
            combo_framedata_df,
            split_columns(combo_input_df, const.MOVE_NAME, " "),
        ]
    )
    combo_framedata_df[const.CHARACTER_NAME] = character_name

    combo_framedata_df = get_frame_data_for_combo(
